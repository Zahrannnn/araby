...
export function ViewOfferModal({ isOpen, onClose, offerId }: ViewOfferModalProps) {
  ...
  const renderServiceDetails = (service: ServiceLineItem) => {
    ...
    // Helper function to render additional costs if they exist
    const renderAdditionalCosts = () => {
      if (service.additionalCosts) {
        if (service.additionalCosts.length > 0) {
          return (
            <div className="mt-4 border-t pt-4">
              <h5 className="font-medium text-gray-900 mb-2">{t('additionalCosts')}</h5>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left font-medium text-gray-500 px-4 py-2">{t('description')}</th>
                      <th className="text-right font-medium text-gray-500 px-4 py-2">{t('amount')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {service.additionalCosts.map((cost, idx) => {
                      // Handle both price and amount field names
                      const costValue = 'price' in cost ? (cost.price as number) : ('amount' in cost ? (cost.amount as number) : 0);
                      
                      return (
                        <tr key={idx} className="border-b">
                          <td className="px-4 py-2 text-gray-900">{cost.description}</td>
          ...
          return (
            <div className="mt-4 border-t pt-4">
              <h5 className="font-medium text-gray-900 mb-2">{t('additionalCosts')}</h5>
              <p className="text-sm text-gray-500 italic">{t('noDetailsAvailable')}</p>
            </div>
          );
        }
      }
        ...
        return (
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              {details.selectedEmployeePlanTariffDescription ? (
                <p><span className="font-medium">{t('tariff')}:</span> {details.selectedEmployeePlanTariffDescription}</p>
              ) : (
                <p><span className="font-medium">{t('tariff')}:</span> {t('notApplicable')}</p>
              )}
              {details.numberOfStaff ? (
              ...
              {details.numberOfDeliveryTrucks ? (
                <p><span className="font-medium">{t('numberOfDeliveryTrucks')}:</span> {details.numberOfDeliveryTrucks}</p>
              ) : (
                <p><span className="font-medium">{t('numberOfDeliveryTrucks')}:</span> {t('notApplicable')}</p>
              )}
              {details.hourlyRateCHF ? (
                <p><span className="font-medium">{t('hourlyRate')}:</span> {formatCurrency(details.hourlyRateCHF)}/h</p>
              ) : (
              ...
              {details.furtherDiscounts ? (
                <p className="md:col-span-2"><span className="font-medium">{t('furtherDiscounts')}:</span> {details.furtherDiscounts}</p>
              ) : (
                <p className="md:col-span-2"><span className="font-medium">{t('furtherDiscounts')}:</span> {t('notApplicable')}</p>
              )}
        ...
        return (
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              {details.hourlyRateCHF ? (
                <p><span className="font-medium">{t('hourlyRate')}:</span> {formatCurrency(details.hourlyRateCHF)}</p>
              ) : (
                <p><span className="font-medium">{t('hourlyRate')}:</span> {t('notApplicable')}</p>
              )}
              {details.durationHours ? (
                <p><span className="font-medium">{t('durationHours')}:</span> {details.durationHours}</p>
              ) : (
                <p><span className="font-medium">{t('durationHours')}:</span> {t('notApplicable')}</p>
              )}
              {details.numberOfStaff ? (
                <p><span className="font-medium">{t('numberOfStaff')}:</span> {details.numberOfStaff}</p>
              ) : (
              ...
              {offerDetails.serviceLineItems.map((service: ServiceLineItem, index: number) => (
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="font-medium text-gray-900">{t(`services.${service.serviceType.toLowerCase()}`)}</h4>
